...
class Field(object):
    ...
    def run_validation(self, data=empty):
        ...
        in the input.
        提供的data可能是一个空的如果输入的数据表示中没有这个数据
        May raise `SkipField` if the field should not be included in the
        validated data.
        如果这个字段不应该被包含在这个validated data里面会抛出一个SkipField
        """
        (is_empty_value, data) = self.validate_empty_values(data)

        if is_empty_value:
            return data
        """这个步骤到底干了什么?不懂,这时候value还是 initial_data吗？"""
        value = self.to_internal_value(data)

        self.run_validators(value)

        """如果上一步没有出现错误,就把value返回给self.validated_data"""
        return value

    def run_validators(self, value):
        """
        Test the given value against all the validators on the field,
        用字段上的所有验证器对字段进行验证，要么抛出ValidationErroe ,要么验证通过简单的返回
