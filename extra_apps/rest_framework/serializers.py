...
class BaseSerializer(Field):
    """
    BaseSerializer提供了最简单的功能，你可以通过集成BaseSerializer来自定义自己的serializer
    需要注意，DRF对不同操作的顺序做了一些限制，谁先谁后有依赖性，可以在serializer上使用以便正确使用
    特别地，
    当BaseSerializer的 'data='参数被传进来时候。    
    .is_valid() - Available.
    ...
    def __init__(self, instance=None, data=empty, **kwargs):
        ...
        """把request.data放到serializer.initial_data"""
    ...
    def save(self, **kwargs):
        ...
        """
        将两个list构建一个字典
        >>> dict([('three': 3), ('two': 2), ('one': 1)])   # 映射函数方式来构造字典
        {'three': 3, 'two': 2, 'one': 1} 
        dict.items()用法
        ...
        """
        # 先生成('a':b)的键值对元祖，然后将两个列表组合到一起，然后再用dict生成dict
        validated_data = dict(
    ...
    def is_valid(self, raise_exception=False):
        ...
        """
        处理is_valid的核心逻辑，如果已经有了validated_data就不处理了。
        把self.initial_data传递到self.run_validation进行检验，如果没问题，返回的值放到self._validated_data
        """
...
class Serializer(BaseSerializer):
    ...
    def run_validation(self, data=empty):
        ...
        我们重写了默认的run_validation(),因为validation应该被验证器执行并且.validate()方法应该必须被写入一个带有
        'non_fields_error' key的错误错误字典中。
